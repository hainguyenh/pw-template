/**
 * ui-actions.ts: This module provides a set of utility functions for performing various actions in Playwright tests.
...
import { INSTANT_TIMEOUT, SMALL_TIMEOUT, STANDARD_TIMEOUT, getDefaultLoadState } from '@utils/constants';
import { getAllLocators, getLocator, getVisibleLocator } from './ui-element';
import { isElementExisted, isElementStability } from './ui-waits';
...
async function getLocatorWithStableAndVisibleOptions(
  ...
  if (options?.stable) await isElementStability(input, options);
  return locator;
}
...
export async function getLocatorCount(input: string | Locator, options?: TimeoutOption): Promise<number> {
  ...
  try {
    if (await isElementExisted(input, { timeout: timeoutInMs })) {
...
/**
 * 2. Interact: Use these functions to interact with ui element such as: click, typing, sendKeys.
 */
 ...
 * @param {TimeoutOption} options - The timeout options.
 */
export async function scrollToView(input: string | Locator, options?: TimeoutOption): Promise<void> {
  const locator = getLocator(input);
